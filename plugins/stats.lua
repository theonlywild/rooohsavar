...
local function run(msg, matches)
  if matches[1]:lower() == 'roohsavar' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /roohsavar ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "roohsavar" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[!/]([Ss]tatslist)$",
    "^[!/]([Ss]tats) (group) (%d+)",
    "^[!/]([Ss]tats) (roohsavar)",-- Put everything you like :)
		"^[!/]([Rr]oohsavar)"-- Put everything you like :)
    }, 
  run = run
}
